...
public class ClienteDAO_IT {
  ...
  public void D3_insert_should_throw_SQLException_when_cliente_already_exists() throws Exception {
    ...
    SQLException ex = Assertions.assertThrows(
            SQLException.class,
  ...
  public void D4_delete_should_throw_SQLException_when_cliente_does_not_exist() throws Exception {
    ...
    Cliente cliente = new Cliente(3, "Pepe", "Garcia");
    cliente.setDireccion("Fake Street");
    cliente.setCiudad("FakeCity");
  ...
  @Test
  public void D5_update_should_modify_cliente_when_cliente_exists() throws Exception {
    IDataSet dataSet = new FlatXmlDataFileLoader().load("/cliente-esperado.xml");
    databaseTester.setDataSet(dataSet);
    databaseTester.onSetup();
  ...
  @Test
  public void D6_retrieve_should_return_cliente_when_cliente_exists() throws Exception {
    IDataSet dataSet = new FlatXmlDataFileLoader().load("/cliente-esperado.xml");
    databaseTester.setDataSet(dataSet);
    databaseTester.onSetup();
    ...
    Assertions.assertEquals("John", cliente.getNombre());
    Assertions.assertEquals("Smith", cliente.getApellido());
    Assertions.assertEquals("1 Main Street", cliente.getDireccion());
    Assertions.assertEquals("Anycity", cliente.getCiudad());
  }
}
